...
const sourceFileRoutes = {
  "Flashforge AD5X, any items..docx": "flashforge-ad5x-any-items.docx",
  "yes, can you also estimate Fired Arts own capabilities in terms of these processes based on information and visuals you gather in research.docx": "yes-capability-assessment.docx"
};

function SourceFileActions({ fileName }) {
  const href = `/research-sources/${encodeURIComponent(sourceFileRoutes[fileName] || fileName)}`;
  return <span className="source-file-actions"><a href={href} target="_blank" rel="noreferrer" aria-label={`Open original ${fileName}`}>Open original</a><a href={href} download={fileName} aria-label={`Download original ${fileName}`}>Download</a></span>;
}
...
function ResearchView({ content, onContentChange, onExport }) {
  ...
  return (
    <div className="view-content workspace-view">
      ...
      <div className="research-tabs" role="tablist" aria-label="Research sections"><button className={tab === "sources" ? "active" : ""} onClick={() => setTab("sources")}>Sources</button><button className={tab === "files" ? "active" : ""} onClick={() => setTab("files")}>Original files</button><button className={tab === "benchmarks" ? "active" : ""} onClick={() => setTab("benchmarks")}>Benchmarks</button><button className={tab === "content" ? "active" : ""} onClick={() => setTab("content")}>Content pack</button></div>
      {tab === "sources" && <><div className="view-toolbar"><SearchBar value={query} onChange={setQuery} placeholder="Search sources and topics" /><FilterBar options={evidenceStatuses} value={filter} onChange={setFilter} /></div><div className="source-grid">{sourceFiltered.map((source) => <article className="source-card" key={source.id}><div className="source-card-head"><span className="source-type">{source.type}</span><SourcePill value={source.evidenceStatus} /></div><h3>{source.topic}</h3><p>{source.note}</p><small>{source.fileName}</small><SourceFileActions fileName={source.fileName} /></article>)}</div></>}
      {tab === "files" && <><div className="view-toolbar"><SearchBar value={query} onChange={setQuery} placeholder="Search original files" /><FilterBar options={evidenceStatuses} value={filter} onChange={setFilter} /></div><div className="source-file-library">{sourceFiltered.map((source) => <article className="source-file-card" key={source.id}><div className="source-card-head"><span className="source-type">{source.type}</span><SourcePill value={source.evidenceStatus} /></div><h3>{source.fileName}</h3><p>{source.topic} · {source.note}</p><SourceFileActions fileName={source.fileName} /></article>)}</div></>}
